...
- Read lane scheduling metadata from the gated `Parallel Lanes` table: `Risk Level`, `Concurrent Group`, `Serial Only`, and `Shared Files`. `Concurrent Group` names lanes safe to launch together; `Serial Only: yes` must use `Concurrent Group: none`; `Shared Files` other than `none` requires `Risk Level: high`.
