...
- Architect observation and architect review roles also run in the background unless their result is the current critical-path blocker; do not serialize independent lanes behind a pending architect subagent.
...
- Read lane scheduling metadata from the gated `Parallel Lanes` table: `Risk Level`, `Concurrent Group`, `Serial Only`, and `Shared Files`. `Concurrent Group` names lanes safe to launch together; `Serial Only: yes` must use `Concurrent Group: none`; `Shared Files` other than `none` requires `Risk Level: critical`.
